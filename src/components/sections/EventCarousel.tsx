...
import { Calendar, SquareArrowOutUpRight, ChevronDown, ChevronUp } from "lucide-react"
    ...
    return (
        <div className='mt-10 mb-20'>
            ...
            <div
                    ...
                    >
                        {extendedSlides.map((slide, index) => (
                            <div key={index} className="relative w-full flex-shrink-0 h-full">
                                ...
                                <div onClick={() => toggleExpanded(index)} className={` overflow-visible p-4 pt-3 absolute top-0 left-0 z-30 rounded-br-md transition-all duration-300 ${expandedSlide === index ? 'h-[100%] w-[100%] backdrop-blur-xs' : 'h-[30%] w-[55%]'}`}>
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="pt-2">
                                                    <a
                                                        href={slide.link}
                                                        target="_blank"
                                                        className="inline-flex items-center text-accent-300 hover:text-accent-400 transition-colors text-sm md:text-base"
                                                    >
                                                        了解更多 <SquareArrowOutUpRight className="ml-1 w-3 h-3" />
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                ...
                <button onClick={goToPrevious} className="text-base md:text-xl lg:text-2xl p-1 md:p-2 pb-2 md:pb-3 absolute top-1/2 -left-2 lg:-left-4 -translate-y-1/2 bg-white/20 rounded-full backdrop-blur-xs z-30">‹</button>
                <button onClick={goToNext} className="text-base md:text-xl lg:text-2xl p-1 md:p-2 pb-2 md:pb-3 absolute top-1/2 -right-2 lg:-right-4 -translate-y-1/2 bg-white/20 rounded-full backdrop-blur-xs z-30">›</button>
            </div>
        </div>
    );
};
